...
def pre_processor(argv):
    ...
    enable_soft_remove = False

    # Get arguments
    try:
        opts = getopt.getopt(argv, 'vc:t:l:d', ['view', 'enable_soft_remove', 'conf=', 'task=', 'log=', 'debug'])[0]
    ...
    for opt,arg in opts:
        if opt in ('-v', '--view'): # View mode (without deleting)
            view_mode = True
        elif opt in ('-s', '--enable_soft_remove'):  # View mode (without deleting)
            enable_soft_remove = True
        elif opt in ('-c', '--conf'):
            conf_path = arg
        elif opt in ('-t', '--task'):
            task = arg
    ...
    try:
        ...
        if task == None: # Task name specified
            for task_name in result:
                try:
                    Task(task_name, result[task_name], not view_mode, enable_soft_remove).execute()
                except Exception:
                    lg.error(traceback.format_exc().splitlines()[-1])
                    lg.error('Task %s fails. ' % task_name)
                    lg.debug('Exception Logged', exc_info=True)
        else:
            Task(task, result[task], not view_mode, enable_soft_remove).execute()
    except Exception:
        lg.error(traceback.format_exc().splitlines()[-1])
        lg.debug('Exception Logged', exc_info=True)
